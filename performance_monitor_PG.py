...
st.title("📊 RAG Chatbot: Scalability & Response Speed Analysis (PostgreSQL)")
...
try:
    ...
    st.sidebar.info(f"🌐 Host: {DB_HOST}:{DB_PORT}")
    
except Exception as e:
    st.error(f"❌ Error: {str(e)}")
    st.error("Please run `setup_db.py` and `ingestion.py` first!")
    st.stop()
