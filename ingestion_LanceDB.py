"""
Document Ingestion for local LanceDB vector benchmark backend.
"""
...
print("\n" + "=" * 80)
print("LANCEDB DOCUMENT INGESTION")
print("=" * 80)
...
def ingest_collection(collection_name: str, documents):
    vector_store = LanceDBVectorStore(LANCEDB_PATH, collection_name, embeddings)
    vector_store.reset()
    uuids = [f"chunk_{i + 1:05d}" for i in range(len(documents))]
    batch_size = 50
    total_batches = (len(documents) + batch_size - 1) // batch_size

    print(f"\nAdding documents to LanceDB collection '{collection_name}'...")
...
print("\n" + "=" * 80)
print("LANCEDB INGESTION COMPLETED")
print("=" * 80)
print(f"\nSummary:")
print(f"   Total chunks created: {len(chunks)}")
print(f"   Vectors stored: {vector_store.count()}")
print(f"   Storage path: {LANCEDB_PATH}")
