...
# Page config
st.set_page_config(
    page_title="ITS Helpdesk Chatbot - ChromaDB",
    page_icon="🤖",
    layout="wide"
)
...
st.caption("Powered by ChromaDB")

# Sidebar
with st.sidebar:
    st.header("⚙️ Configuration")
    st.info(f"**Vector Database:** ChromaDB")
    st.info(f"**Storage:** {CHROMA_PATH}")
...
def init_vector_store():
    ...
    try:
        ...
        if doc_count == 0:
            raise ValueError("Collection is empty. Please run ingestion_chroma.py first.")
...
if st.session_state.vector_store is None:
    with st.spinner("🔄 Loading ChromaDB..."):
        ...
        if st.session_state.vector_store:
            ...
            st.error("❌ Failed to load ChromaDB. Please check your configuration.")
            st.stop()
...
# Footer
st.divider()
st.caption("🎓 ITS Helpdesk Chatbot | Using ChromaDB | Powered by Ollama")